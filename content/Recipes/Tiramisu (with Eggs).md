---
Kind:
  - Recipe
Complexity:
  - Easy
Exertion:
  - Low
tags:
  - Dessert
Link:
Cover:
---
## Ingredients

| Amount    | Name         | Prep                                     |
| --------- | ------------ | ---------------------------------------- |
| (2-3) 3-4 | (large) Eggs | Yolk and egg whites separated in 2 bowls |
| ~50g      | Sugar        |                                          |
| 250g      | Mascarpone   |                                          |
|           | Ladyfingers  |                                          |
|           | Coffee       | Brewed                                   |
|           | Cocoa powder |                                          |

## Instructions

1. Using a hand mixer or machine, beat egg yolks and sugar together for a few minutes until airy, creamy, and light.
2. Using a hand mixer or machine, beat egg whites until it forms rather stiff peaks that don’t bend too much when holding up the whipping device.
3. Add Mascarpone to egg yolk mix and mix for a few more minutes until well combined.
4. Fold in egg white mixture, not over mixing and careful about not crushing or squishing away the fluffiness.

- *Optional step:* Spread a thin layer of cream first to prevent coffee leaks.

3. Fully soak ladyfingers in coffee and create the first layer, aligning all ladyfingers in the same direction and placing them close together.
4. Spread the cream mixture over the ladyfingers to fully cover them.
5. Generously dust with cocoa powder.
6. Create a second layer of coffee-soaked ladyfingers, angled 90 degrees from the previous layer.
7. Spread the cream mixture on top to fully cover ladyfingers.
8. Refrigerate overnight for best results, or at minimum 1-4 hours.
9. Only dust top layer with cocoa powder just before serving.